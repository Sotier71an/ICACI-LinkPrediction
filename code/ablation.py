...
edges_list = []
with open(Path('data/data.txt'), 'r') as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
...
def train_and_evaluate(model, data, train_pairs_tensor, train_labels_tensor, val_pairs, val_labels, 
                       test_pairs, test_labels, lr=0.005, weight_decay=5e-4, num_epochs=1500):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 100 == 0:
            ...
            print(f"Epoch {epoch} | Val Score: {val_score:.4f} | Loss: {loss.item():.4f}")
...
if __name__ == '__main__':
    result_dir = Path('result_ablation')
    ...
    if model_type == 'GCN':
        ...
        model = SAGEEncoder(in_dim, hidden_dim, embed_dim)
    elif model_type == 'GIN':
        model = GINEncoder(in_dim, hidden_dim, embed_dim)
    else:
        raise ValueError("Unknown model type: choose from ['GCN', 'SAGE', 'GIN']")

    print(f"==== 现在使用的模型: {model_type} ====")
    results = train_and_evaluate(
        model, data,
        train_pairs_tensor, train_labels_tensor,
        val_pairs, val_labels,
        test_pairs, test_labels,
        lr=0.005, weight_decay=5e-4, num_epochs=1500
    )
    
    result_file = result_dir / f"{model_type}_results.txt"
    with open(result_file, 'w') as f:
        f.write(f"模型类型: {model_type}\n")
        f.write(f"最佳轮次: {results['best_epoch']}\n")
        f.write(f"测试精确率: {results['precision']:.4f}\n")
        f.write(f"测试召回率: {results['recall']:.4f}\n")
        f.write(f"测试F1分数: {results['f1']:.4f}\n")
        f.write(f"测试AUC: {results['auc']:.4f}\n")
        f.write(f"测试准确率: {results['acc']:.4f}\n")
